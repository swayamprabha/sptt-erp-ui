...
@Injectable()
export class CashEntrySummaryService {
  private _sortReverse: string;
  private _summaryType: string = 'CashEntries';
  ...
  filter(filters: { [key: string]: string[] }): CashEntrySummaryService {
    if (filters) {
      console.log("Filters =>" + JSON.stringify(filters));
    }
    return this;
  }
  sort(sort: { by: string, reverse: boolean }): CashEntrySummaryService {
    if (sort && sort.by) {
